...
/**
 * The one door every brand-scoped mail leaves through.
 *
 * Before this class the send paths of four addons each called
 * `Mail::to(...)->send(...)` or read a global "which mailer" config value. That
 * is the process-wide default mailer with the process-wide From, identically
 * for every brand. The failure is not cosmetic: a relay that verifies sending
 * domains per account replaces a From it does not own, so in a run across
 * several brands the second brand's mail arrives under the first brand's name
 * and address.
 *
 ...
 * cannot bury it.
 *
 * **Subclassing.** An addon binds a subclass typed to its own resolver
 * sub-interface, so a host can answer "who does marketing post come from"
 * differently from "who does transactional post come from". A package with a
 * configured default transport of its own overrides {@see self::transport()};
 * that is the only hook, and it exists because `marketing.sending.mailer` is a
 * documented setting that predates all of this and may not quietly stop
 * working.
 */
class BrandMailer
{
     ...
     * failed. The digest is the reason it exists: the digest builder stamps
     * `digested_at` on every collected item *before* the mail goes out, so a
     * brand that cannot send has to be skipped in front of the recipient loop.
     * Discovering it at the send would mean every recipient's week of items is
     * marked delivered and never resurfaces.
     */
    ...
    public function send(?int $brandId, string $to, ?string $toName, Mailable $mailable): bool
    {
        ...
        //
        // A mailable whose `build()` sets a From unconditionally would undo
        // this. Every mailable that travels this way therefore asks
        // `empty($this->from)` first; that is a property of the mailables, not
        // something this class can enforce, and each one says so at the line.
        ...
        Mail::mailer($this->transport($identity))->send($mailable);
    ...
    public function sendRaw(?int $brandId, ?string $html, ?string $text, callable $build): bool
    {
        ...
        $mailer = Mail::mailer($this->transport($identity));
    ...
    /**
     * Which mailer this identity leaves through.
     *
     * `null` means `config('mail.default')`, which is what `Mail::mailer(null)`
     * has always meant. A package whose own config names a default transport
     * overrides this to fall back to that value instead — a brand that named a
     * mailer still wins, because it is the half of the pair that has to match
     * the address.
     */
