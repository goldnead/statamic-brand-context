 ...
 *
 * This class lives in brand-context rather than in each addon that sends mail
 * because it was, until 12.08.2026, four byte-identical copies with four
 * namespaces. The rule they encode is one rule, and a rule kept in four places
 * is a rule that drifts: the marketing copy had already stopped refusing the
 * transport-without-address pair the other three refuse.
 */
final class SenderIdentity
{
     ...
     *
     * A package that has a configured default transport of its own (marketing's
     * `marketing.sending.mailer`, say) applies it in its own {@see BrandMailer}
     * subclass, not here — see `BrandMailer::transport()`. Putting it here would
     * make the value object package-specific and, worse, would turn the
     * package default into something that outranks a mailable's own From.
     */
